...
def inference(args, multimask_output, db_config, model, test_save_path=None):
    ...
    # 计算每个指标的均值和标准差
    first_mean, first_std = calculate_mean_std(first_list)
    second_mean, second_std = calculate_mean_std(second_list)
    third_mean, third_std = calculate_mean_std(third_list)

    # 计算总体指标的均值和标准差
    overall_metrics = (first_list + second_list + third_list) / 3
    overall_mean, overall_std = calculate_mean_std(overall_metrics)

    # 打印均值和标准差
    print("First Metric (RV): Mean ± Std")
    print(f"Dice: {first_mean[0]:.4f} ± {first_std[0]:.4f}")
    print(f"HD95: {first_mean[1]:.4f} ± {first_std[1]:.4f}")
    print(f"ASD: {first_mean[2]:.4f} ± {first_std[2]:.4f}")
    print(f"JC: {first_mean[3]:.4f} ± {first_std[3]:.4f}")

    print("\nSecond Metric (Myo): Mean ± Std")
    print(f"Dice: {second_mean[0]:.4f} ± {second_std[0]:.4f}")
    print(f"HD95: {second_mean[1]:.4f} ± {second_std[1]:.4f}")
    print(f"ASD: {second_mean[2]:.4f} ± {second_std[2]:.4f}")
    print(f"JC: {second_mean[3]:.4f} ± {second_std[3]:.4f}")

    print("\nThird Metric (LV): Mean ± Std")
    print(f"Dice: {third_mean[0]:.4f} ± {third_std[0]:.4f}")
    print(f"HD95: {third_mean[1]:.4f} ± {third_std[1]:.4f}")
    print(f"ASD: {third_mean[2]:.4f} ± {third_std[2]:.4f}")
    print(f"JC: {third_mean[3]:.4f} ± {third_std[3]:.4f}")

    print("\nOverall Metrics: Mean ± Std")
    print(f"Dice: {overall_mean[0]:.4f} ± {overall_std[0]:.4f}")
    print(f"HD95: {overall_mean[1]:.4f} ± {overall_std[1]:.4f}")
    print(f"ASD: {overall_mean[2]:.4f} ± {overall_std[2]:.4f}")
    print(f"JC: {overall_mean[3]:.4f} ± {overall_std[3]:.4f}")
    ...
    # save:
    # write_csv = "./test_output/save_excel/" + args.exp + "_test_mean.csv"
...
if __name__ == '__main__':
    with torch.cuda.device(1):
        ...
        parser.add_argument('--root_path', type=str,
                        default='/workspace/data/ACDC', help='Name of Experiment')
        ...
        parser.add_argument('--ckpt', type=str, default='/workspace/pretain/sam_vit_b_01ec64.pth',
                            help='Pretrained checkpoint')
        
        parser.add_argument('--lora_ckpt', type=str, default='./check_point/CPPS_SAM_unet_ACDC_3_labels_best_sam_model.pth', help='The checkpoint from LoRA')
