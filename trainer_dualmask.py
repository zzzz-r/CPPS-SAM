...
def trainer_acdc_dualmask_prompt_ssl_fixcoe_random_new_mean_up(args, model1, model2, snapshot_path, multimask_output, low_res):
    ...
    if args.warmup:
        ...
        b_lr = base_lr
    if args.AdamW:
        optimizer1 = optim.AdamW(filter(lambda p: p.requires_grad, model1.parameters()), lr=b_lr, betas=(0.9, 0.999), weight_decay=0.1)
        optimizer2 = optim.AdamW(filter(lambda p: p.requires_grad, model2.parameters()), lr=b_lr, betas=(0.9, 0.999), weight_decay=0.1)
    else:
        optimizer1 = optim.SGD(filter(lambda p: p.requires_grad, model1.parameters()), lr=b_lr, momentum=0.9, weight_decay=0.0001) 
        optimizer2 = optim.SGD(filter(lambda p: p.requires_grad, model2.parameters()), lr=b_lr, momentum=0.9, weight_decay=0.0001)  # Even pass the model.parameters(), the `requires_grad=False` layers will not update
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            outputs1 = model1(image_batch, multimask_output, args.img_size)
            outputs2 = model2(image_batch)

            outputs1 = outputs1['low_res_logits']     
            ...
            if iter_num < args.warm_iter:
                ...
                outputs_round2_model2 = model2(image_batch)
                prompt_masks = outputs_round2_model2
                outputs_round2_model1 = model1(image_batch, multimask_output, args.img_size, prompt_masks)

                outputs_round2_1 = outputs_round2_model1['low_res_logits']
                outputs_round2_2 = outputs_round2_model2
            ...
            if iter_num < args.warm_iter:
                ...
                outputs_round3_model1 = model1(image_batch, multimask_output, args.img_size)
                
                outputs_round3_model2 = model2(image_batch)

                outputs_round3_1 = outputs_round3_model1['low_res_logits']
                outputs_round3_2 = outputs_round3_model2
            ...
            if iter_num > 0 and iter_num % 200 == 0:  # evaluation
                ...
                for i_batch, sampled_batch in enumerate(valloader):
                    metric_i = test_single_volume(
                        sampled_batch["image"], sampled_batch["label"], model1, classes=num_classes+1) 
                ...
                if performance > best_performance:
                    ...
                    save_best = os.path.join(snapshot_path,
                                            'best_model.pth')
                    try:
                        model1.save_lora_parameters(save_best)
                        model1.save_lora_parameters(save_mode_path)
                    except:
                        model1.module.save_lora_parameters(save_best)
                        model1.module.save_lora_parameters(save_mode_path)
            ...
            if iter_num  == args.warm_iter: 
                save_mode_path1 = os.path.join(snapshot_path,'iter_{}_sam.pth'.format(iter_num))
                save_mode_path2= os.path.join(snapshot_path,'iter_{}_unet.pth'.format(iter_num))
                try:
                    model1.save_lora_parameters(save_mode_path1)
                    torch.save(model2.state_dict(), save_mode_path2)
                except:
                    model1.module.save_lora_parameters(save_mode_path1)
