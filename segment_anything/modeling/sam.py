...
class Sam(nn.Module):
    ...
    def __init__(
        ...
        mask_decoder: MaskDecoder_prompt_large,
    ...
    def forward(self, batched_input, multimask_output, image_size, prompt_masks = None):
        if isinstance(batched_input, list):
            outputs = self.forward_test(batched_input, multimask_output)
        else:
            outputs = self.forward_train(batched_input, multimask_output, image_size, prompt_masks)
        return outputs

    def forward_train(self, batched_input, multimask_output, image_size, prompt_masks):
        ...
        if prompt_masks is None:
          sparse_embeddings, dense_embeddings = self.prompt_encoder(
            ...
            points_prompt, points_prompt_random, box_prompt, mask_prompt = self.prompt_generate_random_fast(prompt_masks, image_size, True)
            sparse_embeddings, dense_embeddings = self.prompt_encoder(
